...
public class JwtUtil {
    ...
    static {
        // TODO 后期决定秘钥的存放方式
        PUBLIC_KEY = getPublicKey("MIIBIjANBgkqhkiG9w0BAQEFAAOCAQ8AMIIBCgKCAQEApB2B4q13S1rub2A2AcdI13vam9fj2SEZRVl9W9fC" +
        ...
        PRIVATE_KEY = getPrivateKey("MIIEvQIBADANBgkqhkiG9w0BAQEFAASCBKcwggSjAgEAAoIBAQCkHYHirXdLWu5vYDYBx0jXe9qb1+PZIRl" +
    ...
    public static UserClaim getUserClaim(Jws<Claims> claimsJws) {
        ...
        // 坑货，里面的数据存进去的时候是Long类型，如果数据不够大，取出来给转成Integer了，
        // 丢失原类型，后面反射设置值就很难受了
        Claims body = claimsJws.getBody();
        UserClaim userClaim = new UserClaim();
        Class<? extends UserClaim> aClass = userClaim.getClass();
        ...
        for (Method method : methods) {
            ...
            if (body.get(fieldName) != null) {
                try {
                    method.invoke(userClaim, body.get(fieldName));
                } catch (Exception e) {
                    try {
                        // 失败后先尝试使用Long类型
                        method.invoke(userClaim, Long.valueOf(body.get(fieldName) + ""));
                    } catch (Exception ex) {
                        try {
                            // 再尝试使用String类型，其他类型先不管了
                            method.invoke(userClaim, (body.get(fieldName) + ""));
                        } catch (Exception exc) {
                            exc.printStackTrace();
                        }
